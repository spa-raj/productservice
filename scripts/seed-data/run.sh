...
NAMESPACE="vibevault"
JOB_NAME="seed-products"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"

# Resolve latest image from the running deployment
IMAGE=$(kubectl get deployment productservice -n "$NAMESPACE" -o jsonpath='{.spec.template.spec.containers[0].image}' 2>/dev/null || echo "")

if [ -z "$IMAGE" ]; then
    echo "ERROR: Could not resolve image from productservice deployment."
...
kubectl wait --for=condition=Ready pod -l "purpose=seed-data,app=productservice" -n "$NAMESPACE" --timeout=120s 2>/dev/null || true
